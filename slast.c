...
{ 
    ...
    int req_amount = atoi(argv[FIRST_ARGV]);
    ...
    {
        if (cr.ut_type != DEAD_PROCESS && cr.ut_type != RUN_LVL){ total_amount++; }   
    }
    close (fd);
    ...
    while (read(fd, &cr, reclen) == reclen)
    {
        if (cr.ut_type != DEAD_PROCESS && cr.ut_type != RUN_LVL){ records[i--] = cr; }
    }
    ...
    for (size_t i = 0; (i < req_amount && i < total_amount); i++)
    {
        ...
        records[i];
        long c;
        if ((records[i].ut_type) == BOOT_TIME){
